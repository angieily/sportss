...
def fetch_roster(league,team_id):
    ...
    try:
        return flatten_roster(request_json(url)),None
    except Exception as e:
...
def injury_counts(df):
    ...
    questionable=int(status.str.contains("questionable|game-time|day-to-day",regex=True).sum())
    return {"total":len(df),"out":out,"questionable":questionable}
...
def plain_matchup_factors(game,league,ctx,advanced_rows=None,weather=None):
    ...
    if game.get("neutral_site"):
        ...
        if hs and aws and not pd.isna(hs.get("win_pct",np.nan)) and not pd.isna(aws.get("win_pct",np.nan)):
            diff=hs["win_pct"]-aws["win_pct"]
            if abs(diff)>=.08:
                lean=home["abbr"] if diff>0 else away["abbr"]
                factors.append(
                    f"Home/away split leans {lean}: {home['abbr']} wins {hs['win_pct']*100:.0f}% at home, "
                    f"while {away['abbr']} wins {aws['win_pct']*100:.0f}% on the road."
                )
            else:
                factors.append(
                    f"Home/away split is fairly close: {home['abbr']} home {hs['win_pct']*100:.0f}% vs "
                    f"{away['abbr']} away {aws['win_pct']*100:.0f}%."
                )
    ...
    if h10 and a10:
        factors.append(
            f"Recent form: {home['abbr']} {h10['wins']}-{h10['losses']} with {h10['margin']:+.1f} avg margin; "
            f"{away['abbr']} {a10['wins']}-{a10['losses']} with {a10['margin']:+.1f}."
        )
    ...
    if home.get("injury_verified") and away.get("injury_verified"):
        factors.append(
            f"Verified availability feed: {home['abbr']} has {hi['out']} out/doubtful and "
            f"{hi['questionable']} questionable; {away['abbr']} has {ai['out']} out/doubtful "
            f"and {ai['questionable']} questionable."
        )
    else:
        missing=[]
...
def fetch_injuries(league,team_id):
    ...
    try:
        ...
        if isinstance(payload,dict):
            ...
            if isinstance(direct,list):
                df=_injury_rows_from_items(direct)
                if not df.empty:
                    return df.drop_duplicates(subset=["Player","Status","Detail"]),None
            found=[]
    ...
    try:
        ...
        if matched is not None:
            df=_injury_rows_from_items(matched.get("injuries") or [])
            if df.empty:
    ...
    if not directory.empty and "DISPLAY_FIRST_LAST" in directory.columns:
        ...
        if not exact.empty:
            ...
            return {
                ...
                "is_active":True,
                "source":"live NBA directory",
            },None

        mapping=dict(zip(work["_key"],work["DISPLAY_FIRST_LAST"].astype(str)))
        close=get_close_matches(target,list(mapping.keys()),n=5,cutoff=.58)
        if close:
            display=mapping[close[0]]
            r=work[work["DISPLAY_FIRST_LAST"].astype(str)==display].iloc[0]
            return {
                "id":int(r["PERSON_ID"]),
                "full_name":display,
                "is_active":True,
                "source":"live NBA directory fuzzy match",
            },None

    # Fallback to nba_api's packaged directory.
    try:
        from nba_api.stats.static import players
        ...
        keys={_name_key(p.get("full_name")):p for p in allp}
        close=get_close_matches(target,list(keys.keys()),n=5,cutoff=.58)
        if close:
            return keys[close[0]],None
    except Exception as e:
        return None,str(e)

    return None,err or "No NBA player match found."
...
def match_wnba_player(name, directory):
    ...
    work = directory.copy()
    name_col = "DISPLAY_FIRST_LAST" if "DISPLAY_FIRST_LAST" in work.columns else None
    if not name_col:
        return None
    ...
    names = work[name_col].dropna().astype(str).tolist()
    matches = get_close_matches(str(name), names, n=5, cutoff=.45)
    if matches:
        row = work[work[name_col] == matches[0]]
        if not row.empty:
            return row.iloc[0]
    return None
...
NFL_PATTERNS={
    ...
    "Receptions":[r"RECEIVING REC"],
    "Targets":[r"RECEIVING TGT",r"RECEIVING TAR"],
...
def weighted_mean_std(values,decay=.10):
    ...
    w=np.exp(-decay*np.arange(len(x)))
...
def build_nba_model(mtime):
    ...
    model.fit(train[feats],train["home_won"])
...
def easy_prop_analysis(gamelog,matchup_log,league,prop_spec,side,line,market_probability=None,current_side=None,opponent=""):
    ...
    last5=hit_rate(vals.head(5),side,line)
    last10=hit_rate(vals.head(10),side,line)
    last20=hit_rate(vals.head(20),side,line)
    season=hit_rate(vals,side,line)
...
def statmuse_query_url(league,query):
    slug={"NBA":"nba","WNBA":"wnba","NFL":"nfl"}.get(league,league.lower())
    return f"https://www.statmuse.com/{slug}/ask?" + urlencode({"q":query})

def statmuse_player_links(league,player_name,opponent,today_side):
    links=[]
    if opponent:
        links.append((
            "StatMuse — player vs this opponent",
            statmuse_query_url(
                league,
                f"{player_name} stats vs {opponent} last 10 games"
            ),
        ))
    links.append((
        "StatMuse — last 10 games",
        statmuse_query_url(
            league,
            f"{player_name} stats last 10 games"
        ),
    ))
    if today_side:
        links.append((
            f"StatMuse — {today_side} split",
            statmuse_query_url(
                league,
                f"{player_name} {today_side} stats this season"
            ),
        ))
    return links

def checkmark_prop_table(gamelog,prop_spec,side,line,opponent=""):
    vals=extract_values(gamelog,prop_spec)
    n=min(20,len(gamelog))
    idx=gamelog.index[:n]

    date_col=(
        gamelog.loc[idx,"DATE"].astype(str)
        if "DATE" in gamelog.columns
        else pd.Series(idx.astype(str),index=idx)
    )
    matchup_col=(
        gamelog.loc[idx,"MATCHUP"].astype(str)
        if "MATCHUP" in gamelog.columns
        else gamelog.loc[idx,"OPP"].astype(str)
        if "OPP" in gamelog.columns
    ...
    out=pd.DataFrame({
        ...
        "Stat":pd.to_numeric(vals.loc[idx],errors="coerce").values,
    })
    mask=hit_mask(pd.Series(out["Stat"]),side,line)
    out["Hit?"]=np.where(mask,"✅","❌")
    if opponent:
        out["Vs today's opponent"]=np.where(
            out["Matchup"].astype(str).str.upper().str.contains(
                str(opponent).upper(),regex=False
            ),
            "🎯",
            "",
        )
...
def render_matchup_breakdown_v11(game,league,ctx):
    ...
    if hh and hh.get("games",0):
        ...
        league_slug={"NBA":"nba","WNBA":"wnba","NFL":"nfl"}.get(league,league.lower())
        query=f"{game['home_name']} vs {game['away_name']} last 10 games"
        st.markdown(
            f"[StatMuse: {query}]"
            f"(https://www.statmuse.com/{league_slug}/ask?{urlencode({'q':query})})"
        )
...
def run_free_research(queries,question,context=None):
    """
    Search the public web with Tavily, then optionally summarize with local Ollama.
...
def player_free_research_queries(player_name,game,league):
    ...
    return [
        ...
        f'site:statmuse.com "{player_name}" "{opponent}"',
    ]
...
If a StatMuse result was returned, use it only for facts actually present in the returned snippet/result.
...
def _assistant_sources_for_current_context():
    ...
    if league and player:
        ...
        for label,url in statmuse_player_links(
            league,
            player,
            opponent or "",
            today_side or "",
...
def fetch_basketball_opponent_profile(league, selected_date):
    """
    Opponent team profile from NBA/WNBA Stats when the endpoint responds.
    Useful columns often include OPP_PTS, OPP_FG3M, OPP_FG3A, OPP_FG3_PCT,
    OPP_OREB, OPP_REB, OPP_AST, etc. If the endpoint does not return them,
    the app falls back to advanced pace/defensive-rating signals.
    """
    try:
        from nba_api.stats.endpoints import leaguedashteamstats
        league_id="00" if league=="NBA" else "10"
...
def analyze_bet_line_for_slip(raw,game,league,roster,ctx):
    parsed=parse_freeform_bet(raw,game,league,roster)
    ...
    player_id=""
    if league=="NBA":
        log,p,err=fetch_nba_player_by_name(pname,nba_season_string(selected_date))
        if p:
            player_id=str(p.get("id",""))
        if not err and player_id:
            matchup,_=fetch_basketball_player_matchup_logs(
                "NBA",player_id,opponent_team_id,selected_date,3
            )
    elif league=="WNBA":
        log,p,err=fetch_wnba_player_by_name(pname,season)
        if p:
            player_id=str(p.get("id",""))
        if not err and player_id:
            matchup,_=fetch_basketball_player_matchup_logs(
        ...
        log,err=fetch_espn_player_gamelog_multi(
            "NFL",str(row["athlete_id"]),season,3
        )
        if not log.empty and "OPP" in log.columns:
            matchup=log[
                log["OPP"].astype(str).str.upper().str.contains(
                    str(opponent).upper(),regex=False
                )
            ].copy()
    ...
    if err:
        return {
            ...
            "Break-even":market["probability"] if market else np.nan,
            "Edge":np.nan,"Verdict":"Player data error","Risk":str(err),
        }
    ...
    injuries,inj_err=fetch_injuries(league,team_id)
    ...
    return {
        ...
        "Minutes":minutes_summary_text(minutes),
        ...
        for _,r in frame.iterrows():
            ...
            try:
                if league=="NBA":
                    ...
                    log,p,err=fetch_wnba_player_by_name(name,season)
                else:
                    log,err=fetch_espn_player_gamelog_multi(
...
def opponent_style_exploit_report(
    ...
    for strength in profile["strengths"][:4]:
        caution.append(
...
def render_team_attack_report(game,league,target_side):
    rep=opponent_style_exploit_report(
        game,league,target_side
    )
    st.markdown(
        f"### {rep['target']} betting matchup vs {rep['attacker']}"
    )

    c1,c2=st.columns(2)
    with c1:
        st.markdown("#### 🔴 Team weaknesses that may matter for bets")
        for x in rep["exploit"][:8]:
            st.write("• "+x)
    with c2:
        st.markdown("#### 🟢 Team strengths / reasons the matchup may not translate")
        for x in rep["caution"][:6]:
            st.write("• "+x)

    st.markdown("#### 🎯 Betting impact / player-prop relevance")
    for x in rep["prop_implications"][:5]:
        st.write("• "+x)

    if rep["notes"]:
        with st.expander("Extra matchup notes"):
            for x in rep["notes"][:8]:
                st.write("• "+x)

    with st.expander(f"See {rep['target']}'s full team-profile strengths/weaknesses"):
        st.markdown("**Weaknesses relevant to betting research**")
        if rep["weaknesses"]:
            for x in rep["weaknesses"][:8]:
                st.write("🔴 "+x)
        else:
            st.write("No major weakness crossed the current rank/comparison threshold.")

        st.markdown("**Strengths relevant to betting research**")
        if rep["strengths"]:
            for x in rep["strengths"][:8]:
                st.write("🟢 "+x)
        else:
            st.write("No major strength crossed the current rank/comparison threshold.")
...
def basketball_player_tendency_profile(log):
    ...
    if not strengths:
        strengths.append("no single recent statistical strength crossed the app's current threshold")
    if not weaknesses:
        weaknesses.append("no single recent statistical weakness crossed the app's current threshold")
...
def _full_player_game_data(game,league,row):
    name=str(row["player"])
    abbr=str(row["team_abbr"])
    opponent=game["home_abbr"] if abbr==game["away_abbr"] else game["away_abbr"]
    opponent_team_id=game["home_id"] if abbr==game["away_abbr"] else game["away_id"]
    selected_date=st.session_state.get("selected_date",date.today())
    season=league_season_year(league,selected_date)
    ...
    if league=="NBA":
        ...
        log,err=fetch_espn_player_gamelog_multi(
            "NFL",str(row.get("athlete_id","")),season,3
        )
        if not log.empty and "OPP" in log.columns:
            matchup=log[
                log["OPP"].astype(str).str.upper().str.contains(
    ...
    return {
        ...
        "player_id":player_id,
        "log":log if 'log' in locals() else pd.DataFrame(),
        "matchup":matchup,
        "error":err,
    }

def scan_every_player_for_game(game,league,roster,ctx):
    """
    ...
    for idx,(_,row) in enumerate(roster.reset_index(drop=True).iterrows(),1):
        d=_full_player_game_data(game,league,row)
        name=d["name"]
...
def explain_game_idea(row):
    ...
    lines=[
        ...
        f"Playing status/minutes: **{row.get('Minutes','—')}**",
    ]
    return "\n\n".join(lines)
...
def load_player_v11(row,game,league):
    ...
    season=league_season_year(
        league,selected_date
    )

    with st.spinner(f"Researching {pname} vs {opponent}..."):
        ...
        if league=="NBA":
            ...
            if not err and player_id:
                matchup_log,_=fetch_basketball_player_matchup_logs(
                    "NBA",
                    player_id,
                    opponent_team_id,
                    selected_date,
            ...
            log,player,err=fetch_wnba_player_by_name(
                pname,season
            )
            if player:
                player_id=str(player.get("id",""))
    ...
    if err:
        st.session_state["research_error"]=err
    ...
    st.session_state["research_name"]=player.get(
        "full_name",pname
    )
    ...
    st.session_state["research_team_id"]=str(
        row.get("team_id","")
    )
    st.session_state["research_opponent_team_id"]=str(
        opponent_team_id
    )
...
def v20_render_injuries(game,ctx):
    ...
    for side,col in [("away",left),("home",right)]:
        ...
        with col:
            ...
            if not t.get("injury_verified",False):
                st.warning(
                    "Injury report was not verified. Missing rows do NOT mean nobody is injured."
                )
            else:
                counts=t.get("injuries",{})
                st.write(
            ...
            if inj is not None and not inj.empty:
                ...
                st.success("Verified feed returned no listed players.")
...
def fetch_nflverse_team_stats(season):
    url=(
        ...
        f"stats_team/stats_team_week_{int(season)}.csv"
    )
    try:
        return _remote_csv(url,45),None
    except Exception as e:
        return pd.DataFrame(),f"NFL team stats {season} failed: {e}"

@st.cache_data(ttl=86400, show_spinner=False)
def fetch_nflverse_player_stats(season):
    url=(
        "https://github.com/nflverse/nflverse-data/releases/download/"
        f"stats_player/stats_player_week_{int(season)}.csv"
    )
    try:
        return _remote_csv(url,45),None
    except Exception as e:
        return pd.DataFrame(),f"NFL player stats {season} failed: {e}"
...
def build_nfl_model(current_season):
    ...
    if date_col:
        ...
        m["_date"]=pd.to_datetime(
            ...
            + "-"
            + m["week"].astype("Int64").astype(str),
            errors="coerce",
        )
    ...
    base.fit(train[feats],train["home_won"])

    cal_raw=base.predict_proba(cal[feats])[:,1]
    iso=IsotonicRegression(out_of_bounds="clip")
    iso.fit(cal_raw,cal["home_won"])
...
def build_espn_basketball_model(league,current_season,seasons_back=4):
    ...
    tg["game_key"]=(
        tg["date_dt"].dt.strftime("%Y-%m-%d")
        +"|"
        +tg.apply(lambda r:"|".join(sorted([str(r["team"]),str(r["opponent"])])),axis=1)
    )
...
def build_nba_score_models(mtime):
    ...
    margin_model.fit(train[feats],train["home_margin"])
    total_model.fit(train[feats],train["game_total"])
...
def full_market_checklist_rows():
    return [
        ("Real historical game data — enough seasons to train","✅","NBA local historical database; WNBA multi-season ESPN histories; NFL nflverse historical schedules/team stats."),
        ...
        ("Data collection","✅","NBA/WNBA/ESPN/nflverse/Kalshi/Open-Meteo sources."),
...
@st.cache_data(ttl=600, show_spinner=False)
def fetch_team_recent(league,team_id,season):
    """Robust ESPN team schedule retrieval with multiple legitimate request shapes."""
    cfg=LEAGUES[league]
    url=f"https://site.api.espn.com/apis/site/v2/sports/{cfg['sport']}/{cfg['league']}/teams/{team_id}/schedule"
    attempts=[
    ...
    for params in attempts:
        try:
            payload=request_json(url,params or None,timeout=20)
    ...
    for payload in payloads:
        for event in payload.get("events",[]) or []:
            ...
            comp=comps[0]
            if not _completed_status_v23(event,comp):
                continue
            cs=comp.get("competitors") or []
            mine=next((c for c in cs if str((c.get("team") or {}).get("id",""))==str(team_id)),None)
            if mine is None:
                # Some schedule payloads put team IDs elsewhere. Fallback to first side whose id matches.
                mine=next((c for c in cs if str(c.get("id",""))==str(team_id)),None)
            ...
            rows.append({
                "event_id":str(event.get("id","")),
                "date":event.get("date",""),
                "opponent":(opp.get("team") or {}).get("abbreviation","") or (opp.get("team") or {}).get("shortDisplayName",""),
                "opponent_id":str((opp.get("team") or {}).get("id","")),
                "home_away":mine.get("homeAway",""),
                "result":"W" if won else "L",
                "points_for":pf,
                "points_against":pa,
...
def fetch_event_summary_payload_v23(league,event_id):
    ...
    try:
        return request_json(url,{"event":event_id},timeout=20),None
    except Exception as e:
        return {},f"ESPN game summary failed: {e}"
...
def fetch_injuries(league,team_id,event_id=""):
    """ESPN team + league + selected-game summary fallback. Missing data is never interpreted as healthy."""
    cfg=LEAGUES[league]; team_id=str(team_id)
    frames=[]; messages=[]; verified_sources=[]
    team_url=f"https://site.api.espn.com/apis/site/v2/sports/{cfg['sport']}/{cfg['league']}/teams/{team_id}/injuries"
    try:
        payload=request_json(team_url,timeout=20)
        direct=payload.get("injuries") if isinstance(payload,dict) else None
        if isinstance(direct,list):
            d=_injury_rows_from_items(direct)
            if not d.empty:
                d["Source"]="ESPN team injury feed"; frames.append(d)
        found=[]; _walk_injuries(payload,found)
        d=pd.DataFrame(found)
        if not d.empty:
            d["Source"]="ESPN team injury feed"; frames.append(d)
        verified_sources.append("ESPN team injury feed")
    ...
    try:
        payload=request_json(league_url,timeout=20)
        groups=payload.get("injuries") or []
        ...
        if matched is not None:
            ...
            if not d.empty:
                d["Source"]="ESPN league injury feed"; frames.append(d)
            verified_sources.append("ESPN league injury feed")
    ...
    if event_id:
        ...
        if e: messages.append(e)
            ...
            verified_sources.append("ESPN selected-game summary")
    ...
    if frames:
        ...
        if "Player" in out.columns:
            ...
            # Prefer rows with nonblank status/detail while keeping source provenance.
        ...
        return out,None
    if verified_sources:
        # A source answered successfully but listed no injuries.
        return pd.DataFrame(),None
...
def build_matchup_context(game,league):
    ...
    context={}
    for side in ["home","away"]:
        team_id=game[f"{side}_id"]; abbr=game[f"{side}_abbr"]
        opp=game["away_abbr"] if side=="home" else game["home_abbr"]
        history,herr=fetch_team_history_multi(league,team_id,season,3)
        injuries,ierr=fetch_injuries(league,team_id,game.get("event_id",""))
        h2h,h2h_summary=head_to_head_summary(history,opp,10)
        context[side]={
            "abbr":abbr,"history":history,"history_error":herr,
            "recent5":split_summary(history,n=5),"recent10":split_summary(history,n=10),"season":split_summary(history),
            "home_split":split_summary(history,where="home"),"away_split":split_summary(history,where="away"),
            "h2h":h2h,"h2h_summary":h2h_summary,"rest":rest_days_before_game(history,game.get("date")),
            "injury_df":injuries,"injuries":injury_counts(injuries),"injury_error":ierr,"injury_verified":ierr is None,
        }
    return context
...
def fetch_player_research_v23(row,game,league):
    ...
    frames=[]; notes=[]; player={"full_name":pname}; player_id=""
    if league in {"NBA","WNBA"}:
        years=[]
        if league=="NBA":
            # Season string is based on start year.
        ...
        for y in years:
            try:
                if league=="NBA": d,p,e=fetch_nba_player_by_name(pname,y)
                else: d,p,e=fetch_wnba_player_by_name(pname,int(y))
                if p and not player_id: player_id=str(p.get("id","")); player=p
                if not d.empty: frames.append(_normalize_player_log_v23(d,y,f"{league} official stats"))
                elif e: notes.append(f"{league} official {y}: {e}")
            except Exception as e: notes.append(f"{league} official {y}: {e}")
        # ESPN athlete log is a legitimate fallback when official stats endpoint is empty/blocked.
        if athlete_id and not frames:
            ...
            elif e: notes.append(f"ESPN player fallback: {e}")
    else:
    ...
    if not log.empty:
        log=log.sort_values("_date_sort",ascending=False).drop(columns="_date_sort",errors="ignore")
        # One game only once even when multiple fallbacks overlap.
        dedupe=[c for c in ["EVENT_ID","DATE","MATCHUP","PTS","RESULT"] if c in log.columns]
        if dedupe: log=log.drop_duplicates(subset=dedupe)
    matchup=_exact_opponent_filter_v23(log,opponent)
    return {"log":log,"matchup":matchup,"player":player,"player_id":player_id,"opponent":opponent,"opponent_team_id":opponent_team_id,"notes":notes}


def load_player_v11(row,game,league):
    pname=str(row.get("player","")).strip(); pteam=str(row.get("team_abbr","")).strip(); athlete_id=str(row.get("athlete_id","")).strip()
    with st.spinner(f"Researching {pname} across verified sources..."):
...
def player_research_facts_v23(log,league):
    ...
    # NFL: facts and clear workload weak points only from returned columns.
    strengths=[]; weaknesses=[]; facts=[]
    specs=[("TGT","targets",7,3),("REC","receptions",5,2),("RUSH_ATT","carries",15,6),("CAR","carries",15,6),("PASS_ATT","pass attempts",28,15)]
    for c,label,hi,lo in specs:
        if c not in log.columns: continue
        v=pd.to_numeric(log[c],errors="coerce").head(10).mean()
        if pd.isna(v): continue
        facts.append(f"{v:.1f} {label}/game recently")
        if v>=hi: strengths.append(f"High recent {label} volume ({v:.1f}/game)")
        elif v<=lo: weaknesses.append(f"Low recent {label} volume ({v:.1f}/game)")
    if "PTS" in log.columns:
        v=pd.to_numeric(log["PTS"],errors="coerce").head(10).mean()
        if not pd.isna(v): facts.append(f"{v:.1f} fantasy/score points field when returned")
...
def v20_render_player_style(game,league,log,player_name,position,opponent):
    ...
    with left:
        ...
        if items:
            for x in items[:6]: st.write("• "+str(x))
...
def source_table_v23():
    return pd.DataFrame([
        ...
        ["StatMuse","Direct user-visible verification links only unless a licensed StatMuse API key/product agreement is added","Manual cross-check for player/opponent queries; app does not scrape StatMuse"],
        ["Free public web research","Tavily free search tier + Groq Free API","Searches current public sources; Groq summarizes returned evidence server-side so visitors install nothing"],
...
st.caption(
    "v26 is built for sharing. Visitors only open the website; Tavily web research and Groq Free AI run server-side."
)
...
with st.sidebar:
    ...
    st.caption(
        "Shared-app mode: the owner configures the free research services privately. "
        "Visitors do not install anything or enter API keys."
    )

    if get_tavily_key():
        st.success("Public web search connected")
    else:
        st.warning("Owner setup needed: Tavily key missing")

    if groq_free_available():
        st.success("Free hosted AI connected")
    else:
        st.warning("Owner setup needed: Groq Free key missing")

    st.caption(
        "If a free provider reaches its limit, the app reports that instead of switching to a paid plan."
    )
    ...
    if not week_games:
        ...
        if st.button(
            ...
            with st.spinner(
                ...
                ar,_=fetch_roster(
                    league,chosen["away_id"]
                )
                hr,_=fetch_roster(
                    league,chosen["home_id"]
                )
                if not ar.empty:
            ...
            with st.spinner("Automatically evaluating current winner/spread/total markets for tracking..."):
                try:
            ...
            st.success(
                f"Loaded {chosen['away_abbr']} at {chosen['home_abbr']} "
                f"with {len(roster)} roster players. "
                f"Logged {st.session_state.get('v21_complete_market_logged',0)} new priced market evaluation(s). "
                "Now open Game Report, Player Report, or Find a Bet."
            )
...
with tabs[1]:
    ...
    if not game:
        ...
        render_free_research_block(
            f"v25_game_research::{game.get('event_id','')}",
            "Verified public-source game research",
            game_free_research_queries(game,league),
            game_free_research_question(game,league),
            "v25_game_research_go",
        ...
        for side,col in [
            ...
            with col:
                ...
                if recent:
                    st.write(
                        f"**Last 10:** {recent['wins']}-{recent['losses']} • "
                        f"{recent['avg_for']:.1f} scored • "
                        f"{recent['avg_against']:.1f} allowed • "
                        f"{recent['margin']:+.1f} average margin"
                    )
                else:
                    rec=game.get(f"{side}_records") or {}
                    st.write(
...
with tabs[2]:
    ...
    if not game or roster.empty:
                ...
                if found is not None:
                    ...
                    if clean_name:
                        ...
                        st.warning(
                            ...
                            "The free public-web research below can still research the player instead of stopping here."
                        )
                        if ferr:
        ...
        _free_player_name=st.session_state.get("research_name","")
        if _free_player_name:
            render_free_research_block(
                f"v25_player_research::{game.get('event_id','')}::{_free_player_name.lower()}",
                f"Verified public-source research — {_free_player_name}",
                player_free_research_queries(_free_player_name,game,league),
                player_free_research_question(_free_player_name,game,league),
                "v25_player_research_go",
                context=build_ai_dashboard_context(),
            )
            if log.empty:
                st.info(
                    "No structured game log loaded for this player, but the public-source research above can still return current information and cited evidence."
                )
        ...
        if not log.empty:
            ...
            st.caption("StatMuse links are provided as direct verification searches. The app does not scrape StatMuse; automated StatMuse API access requires licensed credentials under StatMuse's current terms.")
            for label,url in research_source_links(
                game,
                league,
...
with tabs[3]:
    ...
    st.write(
        "This is the automatic bet-finding page. It scans the entire selected game instead of making you choose a player first."
    )
    ...
    if not game or roster.empty:
        ...
        if st.button(
            ...
            with st.spinner(
                f"Researching all {len(roster)} roster players..."
            ):
                ideas,diagnostics,total=scan_every_player_for_game(
                    game,league,roster,ctx
                )
                ranked=v20_top_game_ideas(
                    ideas
                )
            st.session_state[
                "full_game_ideas"
...
with tabs[6]:
    ...
    if not game:
        ...
        render_free_research_block(
            f"v25_market_research::{game.get('event_id','')}",
            "Free public sportsbook + Kalshi research",
            market_free_research_queries(game,league),
            market_free_research_question(game,league),
            "v25_market_research_go",
            context=build_ai_dashboard_context(),
        )
...
with tabs[7]:
    ...
    if hist.empty:
        st.caption(
            ...
            "Open Markets & Kalshi for selected games; those model/market rows are recorded automatically."
        )
    else:
        settled=hist[
...
with tabs[8]:
    ...
    if _g and _c:
        ...
        st.caption(
            "Implemented means the feature exists. Verified means the selected game's data actually loaded or public-source research was run."
        )
    ...
    with st.expander("🤖 Ask — free research assistant"):
        st.caption(
            "Uses the selected-game/player context plus free public web search. "
            "The AI summary runs server-side through the app owner's free Groq account."
        )
        q=st.text_input("Ask about the selected game or player",key="v23_adv_ask")
        if st.button("Research question",key="v23_adv_ask_go") and q:
            _game=st.session_state.get("matchup") or {}
            _league=st.session_state.get("league") or ""
            _player=st.session_state.get("research_name") or ""
            _query_context=" ".join(
                x for x in [
                    str(_league),
                    str(_game.get("away_name","")),
                    str(_game.get("home_name","")),
                    str(_player),
                ] if x
            )
            with st.spinner("Searching public sources..."):
            ...
            if result.get("summary"):
                ...
                st.info("Search results were found. The free hosted AI summarizer is unavailable or has reached a free-tier limit, so the public sources are shown directly.")
            else:
                ans,sources=free_sports_assistant_answer(q)
                st.markdown(ans)
            if result.get("sources"):
    ...
    with st.expander("📚 Sources / research integrity"):
        ...
        st.info("StatMuse remains a verification source/link when returned by public search. The app does not directly scrape StatMuse. Free web discovery uses Tavily; summaries use the owner-configured Groq Free tier.")

    with st.expander(
        "All-sports Kalshi discrepancy scanner"
    ):
        st.caption(
            "This is the technical scanner. The normal selected-game Kalshi view is in Markets & Kalshi."
        )
